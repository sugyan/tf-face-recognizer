...
def inference(images, num_classes, reuse=False):
    def _variable_with_weight_decay(name, shape, stddev, wd=0.0):
    ...
    with tf.variable_scope('fc5', reuse=reuse) as scope:
        ...
        weights = _variable_with_weight_decay('weights', shape=[dim, 150], stddev=0.02, wd=0.001)
    ...
    with tf.variable_scope('fc6', reuse=reuse) as scope:
        weights = _variable_with_weight_decay('weights', shape=[150, 150], stddev=0.02, wd=0.001)
        biases = tf.get_variable('biases', shape=[150], initializer=tf.constant_initializer(0.0))
    ...
    with tf.variable_scope('fc7', reuse=reuse) as scope:
        weights = _variable_with_weight_decay('weights', shape=[150, num_classes], stddev=0.02)
